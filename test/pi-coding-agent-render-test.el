...
(ert-deftest pi-coding-agent-test-open-at-point-browses-visible-url ()
  "C-c C-o opens visible URL text without changing RET file semantics."
  (let ((chat (generate-new-buffer " *pi-visible-url*"))
        (overriding-terminal-local-map nil)
        (overriding-local-map nil)
        (pre-command-hook nil)
        (post-command-hook nil)
    ...
    (unwind-protect
        (save-window-excursion
          ...
          (pi-coding-agent-chat-mode)
          (let ((inhibit-read-only t))
            (insert "See https://example.com/path?q=1."))
          (goto-char (point-min))
          ...
          (cl-letf (((symbol-function 'browse-url)
                    ...
                    ((symbol-function 'find-file)
                     (lambda (&rest _)
                       (ert-fail "URL open visited a file")))
                    ((symbol-function 'find-file-other-window)
                     (lambda (&rest _)
                       (ert-fail "URL open visited a file"))))
            (execute-kbd-macro (kbd "C-c C-o")))
          (should (equal '("https://example.com/path?q=1") calls)))
      (when (buffer-live-p chat)
        (kill-buffer chat)))))

(ert-deftest pi-coding-agent-test-open-at-point-browses-bare-domain-path ()
  "C-c C-o treats visible bare domain paths as HTTPS URLs."
  (let ((chat (generate-new-buffer " *pi-bare-url*"))
        (overriding-terminal-local-map nil)
        (overriding-local-map nil)
        (pre-command-hook nil)
        (post-command-hook nil)
    ...
    (unwind-protect
        (save-window-excursion
          ...
          (pi-coding-agent-chat-mode)
          (let ((inhibit-read-only t))
            (insert "Doc cloud.bytedance.net/docs/goofy/index"))
          (goto-char (point-min))
          (search-forward "bytedance")
          (cl-letf (((symbol-function 'browse-url)
                     (lambda (url &rest _)
                       (push url calls))))
            (execute-kbd-macro (kbd "C-c C-o")))
          (should
           (equal '("https://cloud.bytedance.net/docs/goofy/index") calls)))
      (when (buffer-live-p chat)
        (kill-buffer chat)))))

(ert-deftest pi-coding-agent-test-open-at-point-browses-markdown-url-label ()
  "C-c C-o opens remote Markdown link destinations from their visible label."
  (let ((chat (generate-new-buffer " *pi-markdown-url*"))
        (overriding-terminal-local-map nil)
        (overriding-local-map nil)
        (pre-command-hook nil)
        (post-command-hook nil)
    ...
    (unwind-protect
        (save-window-excursion
          ...
          (pi-coding-agent-chat-mode)
          (let ((inhibit-read-only t))
            (insert "[Docs](cloud.bytedance.net/docs/goofy/index)"))
          (font-lock-ensure)
          (goto-char (point-min))
          (search-forward "Docs")
          (goto-char (match-beginning 0))
          (cl-letf (((symbol-function 'browse-url)
                    ...
                    ((symbol-function 'find-file)
                     (lambda (&rest _)
                       (ert-fail "Markdown URL open visited a file")))
                    ((symbol-function 'find-file-other-window)
                     (lambda (&rest _)
                       (ert-fail "Markdown URL open visited a file"))))
            (execute-kbd-macro (kbd "C-c C-o")))
...
(ert-deftest pi-coding-agent-test-open-at-point-falls-back-to-local-file ()
  "C-c C-o falls back to the RET local file visitor when no URL is present."
  (let ((chat (generate-new-buffer " *pi-open-file-fallback*"))
        (overriding-terminal-local-map nil)
        (overriding-local-map nil)
        (pre-command-hook nil)
        (post-command-hook nil)
    ...
    (unwind-protect
        (save-window-excursion
          ...
          (pi-coding-agent--set-chat-session-identity "/tmp/project/")
          (let ((inhibit-read-only t))
            (insert "docs/report.md"))
          (goto-char (point-min))
          (cl-letf (((symbol-function 'find-file)
                     (lambda (&rest _)
                       (ert-fail "Fallback ignored other-window default")))
                    ((symbol-function 'find-file-other-window)
                     (lambda (path)
                       (push path calls)))
                    ((symbol-function 'browse-url)
                     (lambda (&rest _)
                       (ert-fail "Fallback browsed a URL"))))
            (execute-kbd-macro (kbd "C-c C-o")))
          (should (equal '("/tmp/project/docs/report.md") calls)))
      (when (buffer-live-p chat)
        (kill-buffer chat)))))
